...
language: en
lastReviewedNote: "Reviewed for Portal #71: production brand homepage, shared real-geometry renderer and worker presentation, four-language content, progressive fallback, asset ownership, bundle and rendered verification boundaries."
whenToUse:
...
`src/app/[locale]/page.tsx` loads the existing public summary and renders `BrandHome`. Its search form and all links are server-rendered. `BrandSculpture` is a small client island: an intersection observer starts a dynamic import after hydration when the artwork is visible. It does not include the renderer in the route's initial JavaScript. A same-size SVG silhouette remains on module failure, and the mounted renderer retains model retry, WebGL fallback and complete resource disposal. `useSiteTheme` observes the existing root theme class without adding a preference store.

The production hero renders into an independent OffscreenCanvas inside a same-origin worker when the canvas APIs are available. Completed frames are transferred to a canvas bitmap presenter only after the GPU fence signals, so page composition never waits on an in-flight WebGL frame. At most one frame is in transit, and every frame is consumed or closed. The DOM adapter forwards size, visibility, pointer and appearance changes to the shared `lifecycle-renderer.ts` core; camera, geometry, materials and reflection resolution are identical to the component study. Procedural glow textures also use an offscreen canvas. Page input and navigation remain on the main thread. Unmounting aborts model loading, releases renderer resources and terminates the worker, including a bounded fallback for a stalled context. Unsupported offscreen/bitmap canvas APIs use the main-thread adapter; unavailable WebGL retains the SVG. Prepared Storybook templates use that main-thread adapter for synchronous frozen previews. Worker assets use the existing same-origin CSP, without adding `blob:` or `unsafe-eval` allowances.

`Brand/Homepage` covers four languages, both themes, mobile layout, reduced motion, SVG loading, unavailable artwork, missing summary, production lazy loading and keyboard color/site-theme interaction. Prepared geometry is used for deterministic frozen previews; the lazy-loading story and actual production browser QA verify the deferred path. Keep the original individual-part and reference comparison stories working after shared renderer changes.
