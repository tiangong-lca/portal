...
Canonical `tiangong-lca/portal` 已有可写的 `main`，Portal 也已通过 workspace Issue #759 / PR #760 纳入 `portal` Repo Tag、M1 分支策略、tracked delivery、Docpact 路由与 root integration 流程。这只关闭了“canonical 为空/只读、无法 tracked delivery”的 bootstrap 阻塞；workspace PR #760 仍须在合并前 pin 本仓经审阅的最新 `main` 精确 SHA，不能据此宣称 root integration、R0、R1 或 Production 已就绪。
