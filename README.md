...
lastReviewedNote: "Reviewed for Portal #33: the final public product now targets four independent locales, plain audience-facing copy, performance-first enforcing CSP, public indexing and a retained upstream raw-404 defect without changing the anonymous read-only boundary."
related:
  - AGENTS.md
  - docs/design-plan.md
---
...
Portal #33 正在把现有实现收敛为最终公众产品：`zh-CN`、`en`、`de`、`fr` 四套独立路由与词典没有 UI 回退；首页、搜索、方法说明、候选清单、详情、比较和错误/空态移除开发阶段标签、raw rank/score/reason code 与内部架构话术，并以 TIDAS 术语、Next locale style guide 和 Docs 的公众任务语言为依据。首页、Browse、identifier/keyword Search、Process/Flow 固定版本详情与 Versions、Exchanges、publication-bound LCIA、比较、引用、浏览器候选清单、locale-correct 初始 `<html lang>`、SEO/JSON-LD、品牌、axe/no-JS/forced-colors/200% zoom/四语 Playwright 均保留。Catalog sitemap 继续消费 Database 固定 64 个 opaque shard，但每个 shard 切成 4 个确定性公开 part；每个 catalog index 因此列出 256 项，并为四种语言生成 reciprocal URL，仍严格小于 5 MiB。
...
EdgeOne Production 绑定 `portal/main` 与 `portal.tiangong.earth`，同一环境承担 hosted TDD 和最终发布；feature/PR 不创建独立 Preview。用户已授权公开索引，并选择性能/SEO优先：新 exact main deployment 使用 enforcing CSP，允许 Next App Router 缓存式 hydration 必需的 inline script/style，继续禁止 `unsafe-eval` 并严格限制其他 source/object/base/form/frame 权限。Next/EdgeOne 对未知首段 raw HTML 的 `__next_error__` 包装是真实上游 bug；Portal 保持真实 404、noindex 与原 URL，不以软跳转或牺牲 ISR 的方式伪装修复。索引开关只在 Portal #33 exact Production 四语/SEO/安全/回滚门通过后改为 enabled；上线后的 RUM 改为 7 天非阻断观察。`dppeqhecdjax@82e9edb` 仍是 signer cutover 的不可变证据，品牌 custom/rollback 证据为 `dpx9m06806fi` / `dpzbmb1u15np`。Release #59/#60 由上游继续审核；真实 publication/readback 尚未声称完成。
