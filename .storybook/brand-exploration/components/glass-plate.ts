...
export function createGlassPlate(parent: THREE.Group, layer: number, kit: OpticalKit) {
  ...
  const material = Object.assign(
    new THREE.ShaderMaterial({
      ...
      uniforms: { glassColor: { value: color }, strength: { value: 1 } },
      vertexShader: `
        varying vec2 vUv;
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float sheen = exp(-dot(vUv - vec2(0.86, 0.14), vUv - vec2(0.86, 0.14)) * 3.5);
          float edge = min(min(vUv.x, 1.0-vUv.x), min(vUv.y, 1.0-vUv.y));
          float rim = exp(-edge * 170.0);
          float a = (0.025 + 0.09 * sheen + 0.06 * fresnel + rim * 0.18) * strength;
          gl_FragColor = vec4(glassColor, a);
...
export function createProductPlatform(parent: THREE.Group, kit: OpticalKit) {
  ...
  createGlassPlate(platform, 3, kit);
