...
export async function generateMetadata({
  ...
  const record = mapDataset(
    dataset,
    locale,
    absolutePortalUrl(`/${locale}/flow/${dataset.key.id}@${dataset.key.version}/versions`),
  );
  ...
  return localizedMetadata({
    ...
    locale,
    path: `flow/${record.ref}/versions`,
    title: `${t("versionsTitle")} · ${record.name}`,
  });
}

export default async function FlowVersionsPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string; ref: string }>;
      ...
      {page.nextCursor ? (
        <nav aria-label={common("next")} className="flex justify-end">
          <Button asChild variant="outline">
            <Link
              href={`${localePath(locale, `flow/${dataset.key.id}@${dataset.key.version}/versions`)}?cursor=${encodeURIComponent(page.nextCursor)}`}
            >
              {common("next")}
            </Link>
          </Button>
